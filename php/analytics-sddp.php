<?php
/*
 * Template Name: PSR Analytics SDDP
 */

get_header();
?>

<!--================================
     START HERO AREA
=================================-->
<section class="hero-area text-left">
    <div class="hero-item pt-100 pb-100">
        <!-- Video Background -->
        <video playsinline="playsinline" autoplay="autoplay" muted="muted" loop="loop" class="hero-video">
            <source src="images/video_hero_1.mp4" type="video/mp4">
        </video>

        <!-- Color Gradient Overlay -->
        <div class="overlay-video-sddp"></div><!-- overlay -->

        <div class="container position-relative z-index-1">
            <div class="hero-content" style="display: flex; align-items: center;">
                <img src="images/sddp.svg" alt="Descrição da Imagem" style="width: 90px; height: auto; margin-right: 20px;">
                <div>
                    <h2 class="sec-title text-white font-60" style="margin-right: 400px; font-weight: 700 !important;">SDDP</h2>
                    <p class="sec-title mb-4" style="margin-right: 400px; color: #fff; font-size: 30px;">Energy Systems Operation Planning</p>
                </div>
            </div><!-- end hero-content -->
            <a href="https://www.example.com" class="button-support-link" style="font-weight: 500;">Download SDDP</a>
        </div><!-- container -->
    </div><!-- end hero-item -->
</section><!-- end hero-area -->
<!--================================
    END HERO AREA
=================================-->


<!--================================
     START BAND AREA
=================================-->
<div class="faixa-impacto">
    <p class="frase-impacto">View the latest version <a href="https://example.com" class="link-faixa">release notes</a></p>
</div>
<!--================================
    END BAND AREA
=================================-->

<!--======================================
        START OVERVIEW AREA
======================================-->
<section class="overview-area section----padding">
    <div class="container">
        <div class="row align-items-center">
            <div class="col-lg-4">
                <img src="images/image-sddp-overview.png" alt="feature image" class="img-fluid">
            </div><!-- end col-lg-4 -->
            <div class="col-lg-8">
                <div class="about-content-box mt-5 mt-lg-0 pl-lg-5">
                    <p class="my-4" style="font-size: 25px; line-height: 32px; color: #002846;">
                        <b>SDDP</b> is a stochastic dispatch model for electrical systems with detailed representation of 
                        transmission and fuel networks. It is used for long, medium and short-term operational studies and 
                        is highly flexible in terms of its temporal and spatial levels of detail.
                    </p>
                </div>
            </div><!-- end col-lg-8 -->
        </div><!-- end row -->
    </div><!-- end container -->
</section><!-- end event-area -->
<!--======================================
        END OVERVIEW AREA
======================================-->


<!--======================================
        START CAPABILITIES AREA
======================================-->
<section class="capabilities-area section----padding">
    <div class="container">
        <div class="text-left">
            <h2 class="sec-title" style="font-size: 35px; margin-bottom: 30px; color: #002846;">Modeling Capabilities</h2>
        </div>
            <div class="row">
                <div class="col-lg-4 mb-4">
                    <div class="card-capabilities-sddp">
                        <img src="images/icon-sddp-hydro.png" class="card-capabilities-img-left" alt="Logo TSL Renewable Modeling">
                        <div class="card-content">
                            <p class="card-capabilities-sddp-title"><b>Hydro plants</b></p>
                            <p class="card-capabilities-sddp-body">Variable production coefficient, evaporation, filtration, non-controllable spillage, etc.</p>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4 mb-4">
                    <div class="card-capabilities-sddp">
                        <img src="images/icon-sddp-conf.png" class="card-capabilities-img-left" alt="Logo TSL Renewable Modeling">
                        <div class="card-content">
                            <p class="card-capabilities-sddp-title"><b>Thermal plants</b></p>
                            <p class="card-capabilities-sddp-body">Unit commitment, combined-cycle power plants, nonlinear heat rate, fuel contracts.</p>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4 mb-4">
                    <div class="card-capabilities-sddp">
                        <img src="images/icon-sddp-renewable.png" class="card-capabilities-img-left" alt="Logo TSL Renewable Modeling">
                        <div class="card-content">
                            <p class="card-capabilities-sddp-title"><b>Renewable modeling</b></p>
                            <p class="card-capabilities-sddp-body">Detailed modeling and production of synthetic scenarios for variable renewable energy sources with Time Series Lab.</p>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4 mb-4">
                    <div class="card-capabilities-sddp">
                        <img src="images/icon-sddp-efficiency.png" class="card-capabilities-img-left" alt="Logo TSL Renewable Modeling">
                        <div class="card-content">
                            <p class="card-capabilities-sddp-title"><b>Energy efficiency</b></p>
                            <p class="card-capabilities-sddp-body">Regulations to reduce energy waste and environmental impact by optimizing energy generation.</p>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4 mb-4">
                    <div class="card-capabilities-sddp">
                        <img src="images/icon-sddp-dsm.png" class="card-capabilities-img-left" alt="Logo TSL Renewable Modeling">
                        <div class="card-content">
                            <p class="card-capabilities-sddp-title"><b>Demand Side Management</b></p>
                            <p class="card-capabilities-sddp-body">Response to price, signals by segment by system, area, or bus-level.</p>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4 mb-4">
                    <div class="card-capabilities-sddp">
                        <img src="images/icon-sddp-reliability.png" class="card-capabilities-img-left" alt="Logo TSL Renewable Modeling">
                        <div class="card-content">
                            <p class="card-capabilities-sddp-title"><b>Reliability analysis</b></p>
                            <p class="card-capabilities-sddp-body">Stochastic reliability study focusing on renewable energy and storage using Coral.</p>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4 mb-4">
                    <div class="card-capabilities-sddp">
                        <img src="images/icon-sddp-storage.png" class="card-capabilities-img-left" alt="Logo TSL Renewable Modeling">
                        <div class="card-content">
                            <p class="card-capabilities-sddp-title"><b>Storage devices</b></p>
                            <p class="card-capabilities-sddp-body">Storage capacity, charge and discharge capacities, efficiencies, ramping constraints.</p>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4 mb-4">
                    <div class="card-capabilities-sddp">
                        <img src="images/icon-sddp-reserve.png" class="card-capabilities-img-left" alt="Logo TSL Renewable Modeling">
                        <div class="card-content">
                            <p class="card-capabilities-sddp-title"><b>Operating reserves</b></p>
                            <p class="card-capabilities-sddp-body">Dynamic Probabilistic Reserve (DPR) based on renewable scenarios.</p>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4 mb-4">
                    <div class="card-capabilities-sddp">
                        <img src="images/icon-sddp-maintenance.png" class="card-capabilities-img-left" alt="Logo TSL Renewable Modeling">
                        <div class="card-content">
                            <p class="card-capabilities-sddp-title"><b>Maintenance</b></p>
                            <p class="card-capabilities-sddp-body">User-defined or optimized plant maintenance.</p>
                        </div>
                    </div>
                </div>
            </div>
            <p class="my-4" style="font-size: 24px; font-weight: 300 ; text-align: center;"><b>And much more...</b></p>
       

    </div><!-- end container -->
</section><!-- end team-area -->
<!--======================================
        END CAPABILITIES AREA
======================================-->

<!--======================================
        START TECHNICAL DATA I AREA
======================================-->
<section class="capabilities-area section---padding bgGhostGreen">
    <div class="container">
        <div class="row align-items-center">
            <div class="col-lg-8">
                <div class="about-content-box mt-5pl-lg-5">
                    <h2 class="section-title-model" style="margin-bottom: 30px; ">SDDP finds the <em>optimal operation policy</em> of the system </h2>
                    <p class="card-tools-text">The objective of <b>SDDP</b> is to minimize the sum of costs for purchase and transportation of fuels for thermal plants, 
                        pollutant emission costs, hydro and thermal plants' O&M costs, transmission wheeling rates, cost of energy not supplied and other penalties.  
                        In other words, the model calculates the least-cost operation policy of the system, taking all the aspects above into consideration. </p>
                </div>
            </div><!-- end col-lg-6 -->
            <div class="col-lg-2">
                <img src="images/image-sddp-optimal.svg" alt="feature image" style="width: 300px; height: auto;">
            </div><!-- end col-lg-6 -->
        </div><!-- end row -->
    </div><!-- end container -->
</section><!-- end team-area -->
<!--======================================
        END TECHNICAL DATA I AREA
======================================-->

<!--======================================
        START LICENSING AREA
=======================================-->
<section class="financial-tools-area section--padding bg-image-licensing">
    <div class="container">
        <div class="row align-items-center">
            <div class="col-lg-6">
                <h2 class="get-sddp-title font-40" style="font-size: 40px !important; color:#ffffff">Get SDDP today</h2>
                <a href="https://www.example.com" class="button-support-link" style="font-weight: 500;">Talk to an expert</a>
                <a href="https://www.example.com" class="button-support-link" style="margin-left: 20px; font-weight: 500;">Licensing schemes</a>
            </div>
            <div class="col-lg-6">
                <h2 class="get-sddp-title" style="font-size: 20px; font-weight: 400; margin-bottom: 10px;  color:#fff"> 
                    <b>SDDP</b> is recognized as a best-in-class energy market simulation engine, providing 
                    leading analytical tools and support for professionals such as modelers, power producers, 
                    and analysts. </h2>
            </div>
        </div><!-- end row -->
    </div><!-- end container -->
</section><!-- end event-area -->
<!--======================================
        END LICENSING AREA
=======================================-->

<!--======================================
        START CASE STUDIES AREA
=======================================-->
<section class="capabilities-area section--padding bgGhostWhite">
    <div class="container">
        <div class="text-left">
            <h2 class="section-title-model" style="margin-bottom: 30px; font-weight: 450; ">Find out where <em>SDDP is used</em></h2>
        </div>
        <div class="row mt-5">
                <div class="col-lg-4 col-md-6">
                    <div class="card hover-y">
                        <div class="card-head position-relative">
                            <img src="images/article-panama-1.png" data-src="images/img4.jpg" alt="blog image" class="card-img-top lazy">
                            <span class="position-absolute top-0 left-0 mt-3 ml-3 badge badge-md badge-light"> <i class="la la-calendar"></i> May 14, 2024</span>
                        </div>
                        <div class="card-body">
                            <ul class="list-item">
                                <li class="d-inline-block mr-3"><span class="fal fa-user mr-2 colorBluePSR2 font-14"></span>By <a href="#">PSR Dev</a></li>
                            </ul>
                            <h4 class="card-title mb-4"><a href="blog-single.html"><b>AES Panama</b></a></h4>
                            <p class="card-text my-4">There are many variations of passages of but majority have in some by inject.</p>
                            <a href="blog-single.html" class="btn-link">Continue Reading <i class="fal fa-angle-right ml-1"></i></a>
                        </div><!-- end card-body -->
                    </div><!-- end card -->
                </div><!-- end card -->
                <div class="col-lg-4 col-md-6">
                    <div class="card hover-y">
                        <div class="card-head position-relative">
                            <img src="images/article-canada-2.png" data-src="images/img4.jpg" alt="blog image" class="card-img-top lazy">
                            <span class="position-absolute top-0 left-0 mt-3 ml-3 badge badge-md badge-light"> <i class="la la-calendar"></i> May 14, 2024</span>
                        </div>
                        <div class="card-body">
                            <ul class="list-item">
                                <li class="d-inline-block mr-3"><span class="fal fa-user mr-2 colorBluePSR2 font-14"></span>By <a href="#">PSR Dev</a></li>
                            </ul>
                            <h4 class="card-title mb-4"><a href="blog-single.html"><b>Manitoba Hydro</b></a></h4>
                            <p class="card-text my-4">There are many variations of passages of but majority have in some by inject.</p>
                            <a href="blog-single.html" class="btn-link">Continue Reading <i class="fal fa-angle-right ml-1"></i></a>
                        </div><!-- end card-body -->
                    </div><!-- end card -->
                </div><!-- end card -->
                <div class="col-lg-4 col-md-6">
                    <div class="card hover-y">
                        <div class="card-head position-relative">
                            <img src="images/article-iceland-3.png" data-src="images/img4.jpg" alt="blog image" class="card-img-top lazy">
                            <span class="position-absolute top-0 left-0 mt-3 ml-3 badge badge-md badge-light"> <i class="la la-calendar"></i> May 14, 2024</span>
                        </div>
                        <div class="card-body">
                            <ul class="list-item">
                                <li class="d-inline-block mr-3"><span class="fal fa-user mr-2 colorBluePSR2 font-14"></span>By <a href="#">PSR Dev</a></li>
                            </ul>
                            <h4 class="card-title mb-4"><a href="blog-single.html"><b>Landsnet</b></a></h4>
                            <p class="card-text my-4">There are many variations of passages of but majority have in some by inject.</p>
                            <a href="blog-single.html" class="btn-link">Continue Reading <i class="fal fa-angle-right ml-1"></i></a>
                        </div><!-- end card-body -->
                    </div><!-- end card -->
                </div><!-- end card -->
    </div>
    </div><!-- end container-fluid -->
</section><!-- end testimonial -->
<!--======================================
        END CASE STUDIES AREA
======================================-->

<!--======================================
        START TECHNICAL DATA II AREA
======================================-->
<section class="capabilities-area section--padding bg-image-technical-sddp">
    <div class="container">
        <div class="row align-items-center">
            <div class="col-lg-12">
                <div class="about-content-box mt-5pl-lg-5">
                    <h2 class="section-title-model" style="margin-bottom: 30px; "> PSR solves <em>large-scale multi-stage</em> optimization problems under <em>uncertainty</em></h2>
                    <p class="card-tools-text" style="margin-bottom: 30px; ">SDDP stands for <b>Stochastic Dual Dynamic Programming</b>, an algorithm developed by PSR in 
                        the 1980s for solving large-scale multi-stage optimization problems under uncertainty. 
                        It's not necessary to enumerate the combinations of reservoir levels and the future cost function approximation is 
                        made through a Benders decomposition scheme. </p>
                    <p class="card-tools-text">
                        SDDP algorithm has been extended to several areas and became the global industry standard, 
                        with more than 1.700 citations in the scientific literature. It has been successfully applied 
                        for more than thirty years for mid- and long-term optimal stochastic scheduling of very complex real 
                        systems with multi-scale storage and probabilistic modelings such as hydro inflows, 
                        renewable intermittency, demand, and fuel prices. </p>
                </div>
                <a href="#" class="analytics-btn analytics-btn-primary" style="margin-top: 30px; font-size: 22px;" id="downloadBtn">Download SDDP folder</a>
            </div><!-- end col-lg-6 -->
        </div><!-- end row -->
    </div><!-- end container -->
</section><!-- end team-area -->
<!--======================================
        END TECHNICAL DATA II AREA
======================================-->

<!-- Configura pop-up de informações necessárias para fazer download do folder -->
<div id="popupForm" class="popup-form">
    <div class="popup-form-content">
        <span class="close">&times;</span>
        <h2>You are almost there!</h2>
        <form id="downloadForm">
            <label for="name">Name:</label>
            <input type="text" id="name" name="name" required>

            <label for="email">E-mail:</label>
            <input type="email" id="email" name="email" required>

            <label for="company">Company:</label>
            <input type="text" id="company" name="company" required>

            <label for="country">Country:</label>
            <input type="text" id="country" name="country" required>

            <label for="phone">Phone number:</label>
            <input type="tel" id="phone" name="phone" required>

            <button class="analytics-btn analytics-btn-primary" style="margin-top: 30px; font-size: 16px;" type="submit">Download</button>
        </form>
    </div>
</div>

<!--======================================
        START EXTRA TOOLS AREA
======================================-->
<section class="financial-tools-area section--padding bgGhostGreen">
    <div class="container">
        <div class="row align-items-center">
            <div class="col-lg-4 align-items-center">
                <div class="text-left" style="margin-right: 20px;">
                    <h2 class="section-title-model font-40" style="margin-bottom: 20px; font-weight: 600;"> 
                        <em>Included complementary tools</em>
                    </h2>
                    <p class="font-weight-100" style="font-size: 24px;">
                        The SDDP license comes with a suite of additional tools designed to enhance your experience and productivity.
                    </p>
                </div>
            </div><!-- end col-lg-4 -->
            <div class="col-lg-8 align-items-self">
                <div class="row align-items-center">
                    <div class="col-md-6 mb-4">
                        <div class="card-extra-tools position-relative"> <!-- Adicione position-relative para o stretched-link funcionar -->
                            <a href="URL_DO_SEU_LINK" class="stretched-link"></a>
                            <img src="images/tsl_blue.svg" class="card-extra-tools-img-left" style="width: 14%;" alt="Logo TSL Renewable Modeling">
                            <div class="card-extra-tools-body">
                                <h5 class="card-extra-tools-title">Time Series Lab</h5>
                                <p class="card-text">Renewable Modeling Tool</p>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6 mb-4">
                        <div class="card-extra-tools position-relative"> <!-- Adicione position-relative para o stretched-link funcionar -->
                            <a href="URL_DO_SEU_LINK" class="stretched-link"></a>
                            <img src="images/psrio_dark.svg" class="card-extra-tools-img-left"  alt="Logo TSL Renewable Modeling">
                            <div class="card-extra-tools-body">
                                <h5 class="card-extra-tools-title">PSRIO</h5>
                                <p class="card-text">B.I. for Data Analysis</p>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6 mb-4">
                        <div class="card-extra-tools position-relative"> <!-- Adicione position-relative para o stretched-link funcionar -->
                            <a href="URL_DO_SEU_LINK" class="stretched-link"></a>
                            <img src="images/psrcloud_blue.svg" class="card-extra-tools-img-left" style="width: 14%;" alt="Logo TSL Renewable Modeling">
                            <div class="card-extra-tools-body">
                                <h5 class="card-extra-tools-title">PSR Cloud</h5>
                                <p class="card-text">Cloud Computing Environment</p>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6 mb-4">
                        <div class="card-extra-tools position-relative"> <!-- Adicione position-relative para o stretched-link funcionar -->
                            <a href="URL_DO_SEU_LINK" class="stretched-link"></a>
                            <img src="images/coral.png" class="card-extra-tools-img-left" alt="Logo TSL Renewable Modeling">
                            <div class="card-extra-tools-body">
                                <h5 class="card-extra-tools-title">Coral</h5>
                                <p class="card-text">Reliability Analysis Tool</p>
                            </div>
                        </div>
                    </div>
                </div>
            </div><!-- end col-lg-8 -->
        </div><!-- end row -->
    </div><!-- end container -->
</section><!-- end event-area -->
<!--======================================
        END EXTRA TOOLS AREA
======================================-->

<!--======================================
        START SUPPORT AREA
=======================================-->
<section class="capabilities-area section----padding bgDarkBlue">
    <div class="container">
        <div class="row align-items-center">
            <div class="col-lg-6">
                <div class="about-content-box mt-5">
                    <h2 class="section-support-title" style="margin-bottom: 30px; font-weight: 600;">Need help?</h2>
                    <p class="section-support-text" style="margin-bottom: 30px;">
                        As an active user of PSR solutions, you have access to our support services and to our Knowledge Hub, 
                        a platform in which you can find documentation, frequently asked questions and detailed methodology.
                    </p>
                </div>
            </div><!-- end col-lg-6 -->
            <div class="col-lg-6">
                <div class="row align-items-center"> <!-- Adicionado uma nova row para os cards -->
                    <div class="col-lg-6 mb-4"> <!-- Alterado para col-lg-6 para alinhar os cards lado a lado -->
                        <div class="card-support text-center hover-y">
                            <a href="URL_DO_SEU_LINK" class="stretched-link"></a>
                            <div class="card-solution-body">
                                <img src="images/icon-support.png" class="card-solution-img" alt="SDDP">
                                <h5 class="card-solution-title mb-0">Support</h5>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-6 mb-4"> <!-- Alterado para col-lg-6 para alinhar os cards lado a lado -->
                        <div class="card-support text-center hover-y">
                            <a href="URL_DO_SEU_LINK" class="stretched-link"></a>
                            <div class="card-solution-body">
                                <img src="images/icon-kw.png" class="card-solution-img" alt="OptGen">
                                <h5 class="card-solution-title mb-0">Knowledge Hub</h5>
                            </div>
                        </div>
                    </div>
                </div><!-- end inner row -->
            </div><!-- end col-lg-6 -->
        </div><!-- end row -->
    </div><!-- end container -->
</section><!-- end capabilities-area -->
<!--======================================
        END SUPPORT AREA
=======================================-->

<?php get_footer(); ?>